...
import { useNounTokens } from "../hooks/tokens.js";
...
const NounsToken = ({ context }) => {
  const { address: connectedAccount } = useAccount();
  return (
    <>
      {connectedAccount != null && (
        <div style={{ marginBottom: "4.8rem" }}>
          <ConnectedAccountSection context={context} />
        </div>
      )}

      {context !== "nouns-governor" && (
...
const useOwnedNounIds = (address) => {
  const nouns = useNounTokens(address);
  return nouns?.map((n) => n.id);
};
...
const ConnectedAccountSection = ({ context }) => {
  const { address: connectedAccount } = useAccount();

  const nounIds = useOwnedNounIds(connectedAccount);
