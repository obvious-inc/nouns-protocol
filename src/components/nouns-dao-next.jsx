...
import {
  useNounsDaoNextWrite as useNounsDaoWrite,
  useNounsDaoNextRead as useNounsDaoRead,
  useNounsDaoNextReads as useNounsDaoReads,
...
const NounsDao = () => {
  ...
  const daoContractAddress = useAddress("nouns-dao");

  const proposalIds = useProposalIds({ order: "desc" });

  if (daoContractAddress == null) return null;
...
const Proposals = ({ proposalIds }) => {
  ...
  const { data: voteReceiptResponses } = useNounsDaoReads("votingReceipt", {
