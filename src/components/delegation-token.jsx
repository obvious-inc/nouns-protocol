...
const MintDelegationTokenForm = ({ ownedNounTokens, delegationTokens }) => {
  ...
  const isDelegated = (tokenId) =>
    ownedNounTokens.some((t) => t.id === tokenId && t.delegate != null);
  ...
  const { call: mintBatch } = useDelegationTokenWrite("mintBatch", {
    ...
    enabled:
      ...
      targetTokenIds.every((id) => !isDelegated(id)),
    watch: true,
  });
  ...
  return (
    ...
    >
      ...
      <MultiSelect
        ...
        options={[
          ...
          .map((t) => {
            ...
            if (t.delegate != null)
              label += ` (delegated to ${truncateAddress(t.delegate)})`;
            if (t.owner != null) {
              if (t.owner === connectedAccount) {
                label += " (delegated to yourself)";
              } else {
      ...
      >
        ...
        <button type="submit" disabled={submitCall == null}>
          {(() => {
            switch (primaryAction) {
              case "mint-batch":
                return "Mint delegation token batch";
              case "mint-single":
      ...
      {(() => {
        if (targetTokenIds.length > 1 && targetTokenIds.some(isDelegated))
          return (
            <div data-small data-warning style={{ marginTop: "1em" }}>
              One or more of the selected nouns already has a minted delegation
              token.
            </div>
          );
        return null;
